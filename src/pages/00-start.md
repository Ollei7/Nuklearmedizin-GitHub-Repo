---
id: start
title: Nuklearmedizin – Strahlung, die heilt?
nav: Startseite
group: Start
order: 0
badge: Lerneinheit Physik · Jahrgang 9/10
---

<div class="nojs-note">Hinweis: Du siehst gerade alle Seiten untereinander (z. B. in der Vorschau der Dateien-App). Mit dem Menü springst du zu den einzelnen Seiten. Öffnest du die Datei in Safari (über IServ) oder in einer Browser-App, wird jede Seite einzeln angezeigt und deine Checklisten werden gemerkt.</div>

Fast jede und jeder von euch war schon einmal beim Röntgen – beim Zahnarzt, nach einem Sturz, vielleicht im CT. Und viele kennen jemanden, der mit Strahlung untersucht oder behandelt wurde: eine Schilddrüsen-Szintigraphie, eine Bestrahlung, eine Radioiodtherapie. Dabei stellen sich immer dieselben Fragen: **Was macht Strahlung im Körper? Wie schädlich ist sie? Wie schützt man sich? Wird man „verstrahlt“ – und wie lange „strahlt“ man nach einer Behandlung?**

Um diese Fragen zu beantworten, braucht man Physik: Strahlungsarten, Halbwertszeit, Abschirmung, Abstand, Dosis. Genau das erarbeitet ihr in dieser Einheit – nicht abstrakt, sondern an **zwei Fallgeschichten aus der Nuklearmedizin**: Frau A., die eine Szintigraphie bekommt, und Frau B., die mit radioaktivem Iod behandelt wird. Am Ende führt ihr selbst ein Arzt-Patienten-Gespräch und bewertet, ob eine umstrittene Therapie sinnvoll ist.

## Die Module

<div class="tiles" markdown="1">
<a class="tile m0" href="#modul0"><b>Modul 0</b>Warum trägt man beim Zahnarzt eine Bleischürze?<small>1,5 Doppelstunden · Erklärkarte fürs Wartezimmer</small></a>
<a class="tile m1" href="#modul1"><b>Modul I</b>Wie schützt man sich vor Strahlung? Die 5 A<small>1,5 Doppelstunden · Wandkarten zum Strahlenschutz</small></a>
<a class="tile m2" href="#modul2"><b>Modul II</b>Fallgeschichten: Szintigraphie oder Radioiodtherapie<small>4 Doppelstunden · Patienteninformation (zwei Pfade)</small></a>
<a class="tile m3" href="#modul3"><b>Modul III</b>Das Arzt-Patienten-Gespräch<small>1 Doppelstunde · Gesprächsleitfaden und Rollenspiel</small></a>
<a class="tile m4" href="#modul4"><b>Modul IV</b>Radon – Gefahr oder Heilmittel?<small>1 Doppelstunde · Begründete Empfehlung</small></a>
</div>

## So funktioniert die Arbeit

Ihr arbeitet in festen **Teams** (4–5 Personen). Jedes Modul läuft nach demselben Muster ab – das nennen wir **Sprint**:

<div class="sprintplan">
<div><b>1 · Aufgabe</b>Leitfrage, Warum, Arbeitspakete und Kriterien verstehen</div>
<div><b>2 · Sprint-Planung</b>Aufgaben auf dem Scrumboard verteilen</div>
<div><b>3 · Arbeitsphase</b>Zu Beginn jeder Stunde ein kurzes Stand-up</div>
<div><b>4 · Review</b>Lernprodukt abgeben oder vorstellen</div>
<div><b>5 · Retrospektive</b>Kurz zurückblicken: Was lief gut, was ändern wir?</div>
</div>

Wie das genau geht, steht auf der Seite [So arbeiten wir – Teamarbeit & Scrum](#agil). Lest sie **vor Modul 0** gemeinsam durch.

!!! tipp "Tipps zur Nutzung dieser Seite"
    - Über das **Menü** kommst du zu jedem Modul, zu allen Infotexten und Arbeitsblättern.
    - **Checklisten** kannst du antippen. Wird die Seite in Safari (z. B. über IServ) geöffnet, merkt sich das iPad deine Häkchen.
    - Alle Bilder und Formeln sind in dieser einen Datei enthalten – du brauchst kein Internet. Nur die Simulationen (GeoGebra), die AR-App und einige Broschüren führen nach draußen.

## Was ihr am Ende könnt

Am Ende der Einheit könnt ihr erklären, warum Röntgen- und radioaktive Strahlung Zellen schädigen kann, mit welchen fünf Regeln man sich schützt, wie eine Szintigraphie und eine Radioiodtherapie physikalisch funktionieren, was Halbwertszeit, Zerfallsreihe und effektive Dosis bedeuten – und ihr könnt einer besorgten Patientin verständlich und ehrlich erklären, worauf sie sich einlässt.
